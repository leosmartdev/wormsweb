...
function MarketPlacePage() {
  ...
  function calculateMintedEggs() {
    window.contract = new web3.eth.Contract(Config.MintContract.abi, Config.MintContract.address);
  ...
  const mint_NFT = async (values) => {
    ...
    const mintContract = await new web3.eth.Contract(Config.MintContract.abi, Config.MintContract.address);

    const BusdContract = await new web3.eth.Contract(Config.BusdContract.abi, Config.BusdContract.address)
    const busdVal = values
    const value = web3.utils.toWei(busdVal)
    const approve = await BusdContract.methods.approve(Config.MintContract.address, value).send({ from: walletAddress })

    if (approve) {
      const transactionParameters = {
        to: Config.MintContract.address,
        from: account1,
        value: web3.utils.toHex(10e16),
        data: mintContract.methods.safeMint(walletAddress).encodeABI(),
      };
